...
function noo_customizer_add_custom_controls( $wp_customize ) {
	...
	//
	// Checkbox Multiple control of WP Customizer.
	//
	class NOO_Customizer_Control_CheckboxMultiple extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$multi_values = !is_array( $multi_values ) ? explode( ',', $multi_values ) : $multi_values; 
			?>
				<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
			<?php foreach ( $this->choices as $value => $label ) : ?>
				<label>
					<input type="checkbox" value="<?php echo esc_attr( $value ); ?>" name="<?php echo esc_attr( $name ); ?>[]" <?php checked( in_array( $value, $multi_values ) ); ?> />
		...
		public function __construct( $manager, $id, $args = array(), $options = array() ) {
			...
			if ( empty ( self::$font_weight_name ) )
				self::$font_weight_name = array(
					...
					'regular'   => __( 'Regular', 'noo' ),
					'italic' 	=> __( 'Regular Italic', 'noo' ),
					'500'       => __( 'Medium', 'noo' ),
