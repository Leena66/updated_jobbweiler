	...
	function jm_job_package_cf_features( $product ) {
		...
		foreach ( $all_job_package_cfs as $field_name ) :
			...
			if( in_array( $field_name, $package_cfs ) ) : ?>
				<li class="noo-li-icon"><i class="fa fa-check-circle"></i> <?php echo sprintf( __('Post job with %s','noo'), $field_label );?></li>
			<?php else : ?>
				<li class="noo-li-icon"><i class="fa fa-times-circle-o not-good"></i> <?php echo sprintf( __('No %s for Jobs','noo'), $field_label );?></li>
			<?php endif; ?>
		<?php endforeach;
	}
