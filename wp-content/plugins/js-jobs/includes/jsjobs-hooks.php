...
function jsjobs_custom_css_add() {
    echo '<link rel="stylesheet" href="' . jsjobs::$_pluginpath . 'includes/css/adminmenu.css' . '" type="text/css" media="all" />';
}
...
function jsjobs_add_registration_fields() {
    ...
    if (isset($_SESSION['js_cpfrom'])) {
        ...
        ?>
        <p>
            <label for="jobs_role"> <?php _e('Jobs role', 'js-jobs') ?><br />
                <select id="jobs_role" name="jobs_role" class="input form-control jsjb-jm-select-field">
                    <option value="0"><?php echo __('Select job role', 'js-jobs'); ?></option>
...
function jsjobs_registration_save($user_id) {
    if (isset($_POST['jobs_role'])) {
        $role = $_POST['jobs_role'];
        $user_email = $_POST['jsjobs_user_email'];
        if (is_numeric($role)) {
            if ($role == 1) {
...
function jsjobs_add_new_member() {
    if (isset($_POST["jsjobs_user_login"]) && wp_verify_nonce($_POST['jsjobs_jobs_register_nonce'], 'jsjobs-jobs-register-nonce')) {
        $user_login = $_POST["jsjobs_user_login"];
        $user_email = $_POST["jsjobs_user_email"];
        $user_first = $_POST["jsjobs_user_first"];
        $user_last = $_POST["jsjobs_user_last"];
        $user_pass = $_POST["jsjobs_user_pass"];
        $pass_confirm = $_POST["jsjobs_user_pass_confirm"];
        ...
        if (empty($errors)) {
            ...
            if ($new_user_id) {
                ...
                if ($userrole == 'employer') {
                    ...
                    JSJOBSMessages::setLayoutMessage(__('User Added Successfully', 'js-jobs'), 'updated','user');
