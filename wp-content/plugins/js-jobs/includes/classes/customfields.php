...
class JSJOBScustomfields {
    ...
    function formCustomFieldsResume($field , $obj_id, $obj_params ,$resumeform=null,  $section = null, $sectionid = null, $ishidden = null,$themecall=null){
        ...
        $themebfclass=($themecall==1)?" jsjb-jm-bigfont":"";        
        if(null != $themecall){
        ...
        switch ($field->userfieldtype) {
            case 'text':
            case 'email':
                $themeclass=($themecall==1)?getJobManagerThemeClass('text'):"";
            ...
            case 'date':
                    ...
                    $extraattr = array('class' => 'inputbox custom_date cal_userfield  '.$themeclass.' '.$cssclass, 'size' => '10', 'maxlength' => '19','data-validation'=>$req);
            ...
            case 'textarea':
                ...
                $themeclass=($themecall==1)?getJobManagerThemeClass('textarea'):"";
            ...
            case 'checkbox':
                if (!empty($field->userfieldparams)) {
                    ...
                    foreach ($obj_option AS $option) {
                        ...
                        $user_field .= '<input type="checkbox" ' . $check . ' '.$data_required.' class="radiobutton '.$cssclass.'" value="' . $option . '" id="' . $id . '_' . $i . '" name="' . $name . '[]">';
            ...
            break;
            case 'combo':
                $themeclass=($themecall==1)?getJobManagerThemeClass('select'):"";            
                $comboOptions = array();
                if (!empty($field->userfieldparams)) {
                    $obj_option = json_decode($field->userfieldparams);
            ...
            case 'depandant_field':
                ...
                /*echo '<pre>';var_dump($obj_params);echo '</pre>';
                echo '<pre>';var_dump($field->field);echo '</pre>';
                echo '<pre>';var_dump($valuearray);echo '</pre>';
                exit;*/
                //code for handling dependent field
                $jsFunction = '';
                if ($field->depandant_field != null) {
    ...
    function formCustomFields($field,$resumeform = null, $section = null, $refid = null,$themecall=null) {
        ...
        if ($resumeform == 1) {
            ...
            $userfielddataarray = json_decode(jsjobs::$_data[0]->params);
            $uffield = $field->field;
        ...
        switch ($field->userfieldtype) {
            ...
            case 'email':
                $themeclass=($themecall==1)?getJobManagerThemeClass('text'):"";
                $html .= JSJOBSformfield::text($field->field, $value, array('class' => ' inputbox one '. $themeclass, 'data-validation' => $cssclass, 'size' => '.$field->size.', $maxlength, $readonly));
                break;
            case 'date':
                $themeclass=($themecall==1)?getJobManagerThemeClass('text'):"";
                $html .= JSJOBSformfield::text($field->field, $value, array('class' => 'custom_date one '. $themeclass, 'data-validation' => $cssclass));
                break;
            case 'textarea':
                $themeclass=($themecall==1)?getJobManagerThemeClass('textarea'):"";
            ...
            case 'radio':
                ...
                $html .= JSJOBSformfield::radiobutton($field->field, $comboOptions, $value, array('data-validation' => $cssclass , 'class' =>  $dependentclass, 'onclick' => $jsFunction));
                break;
            case 'combo':
                $themeclass=($themecall==1)?getJobManagerThemeClass('select'):"";
                $comboOptions = array();
    ...
    function formCustomFieldsForSearch($field, &$i, $resumeform = null, $subrefid = null,$themecall=null,$themrefine=null) {
        ...
        if($resumeform != 3){// to handle top search case for job and resume listing.
            $themebfclass=($themecall==1)?" jsjb-jm-bigfont":"";
            $themenopadmarclass=($themrefine==1)?" jsjb-jm-nopad-nomar":"";
            ...
            $html = '<div class="' . $div1 . '" title="'. __($field->fieldtitle,'js-jobs') .'" >
                   <div class="' . $div2 . '">';
            $html .= __($field->fieldtitle,'js-jobs');
            $html .= ' </div><div class="' . $div3 . '">';
        }
        ...
        switch ($field->userfieldtype) {
            ...
            case 'email':
                $themeclass=($themecall==1)?getJobManagerThemeClass('text'):"";
                $html .= JSJOBSformfield::text($field->field, $value, array('class' => 'inputbox one form-control jsjb-jm-input'.$themeclass, 'data-validation' => $cssclass, 'size' => $field->size, $maxlength, $readonly, 'placeholder' => __($field->fieldtitle, 'js-jobs')));
                break;
            case 'date':
                $themeclass=($themecall==1)?getJobManagerThemeClass('text'):"";
                $html .= JSJOBSformfield::text($field->field, $value, array('class' => 'custom_date one '.$themeclass, 'data-validation' => $cssclass));
            ...
            case 'combo':
                ...
                //end
                $html .= JSJOBSformfield::select($field->field, $comboOptions, $value, __('Select', 'js-jobs') . ' ' . $field->fieldtitle, array('data-validation' => $cssclass, 'onchange' => $jsFunction, 'class' => 'inputbox one form-control  jsjb-jm-select '.$themeclass));
                break;
            case 'depandant_field':
                $themeclass=($themecall==1)?getJobManagerThemeClass('select'):"";            
            ...
            case 'multiple':
                ...
                $array = $field->field;
                $array .= '[]';
                $html .= JSJOBSformfield::select($array, $comboOptions, $value, __('Select', 'js-jobs') . ' ' . $field->fieldtitle, array('data-validation' => $cssclass, 'multiple' => 'multiple','class'=>'inputbox one form-control  jsjb-jm-select '.$themeclass));
                break;
        }
        if ($resumeform == 3) {// to handle top search case for job and resume listing.
            return $html;
    ...
    function showCustomFields($field, $fieldfor, $params) {
        ...
        if($fieldfor == 1){ // jobs listing
            ...
            if ($labelflag == 1) {
                $html .= '<span class="js-bold">' . $field->fieldtitle . ':&nbsp</span>';
            }
            $html .= '<span class="get-text">' . $fvalue . '</span>
                         </div>';
        }elseif($fieldfor == 2){ // job view
            $html = '<div class="detail-wrapper"  >
                        <span class="heading">' . $field->fieldtitle . ':&nbsp</span>
