...
class JSJOBSjobModel {
    ...
    function getJobbyIdForView($job_id) {
        ...
        //echo "<pre>";var_dump(jsjobs::$_data['relatedjobs']);        
    ...
    function sorting() {
        ...
        if($pagenum > 1 && isset($_SESSION['jobs'])){
            jsjobs::$_data['sorton'] = $_SESSION['jobs']['sorton'];
            jsjobs::$_data['sortby'] = $_SESSION['jobs']['sortby'];
        }else{
            $_SESSION['jobs']['sorton'] = jsjobs::$_data['sorton'];
            $_SESSION['jobs']['sortby'] = jsjobs::$_data['sortby'];
        }
    ...
    function getAllJobs() {
        ...
        $location = JSJOBSrequest::getVar('location');
        ...
        if ($formsearch == 'JSJOBS_SEARCH') {
            ...
            $_SESSION['JSJOBS_SEARCH']['location'] = $location;
        }
        if (JSJOBSrequest::getVar('pagenum', 'get', null) != null) {
            ...
            $location = (isset($_SESSION['JSJOBS_SEARCH']['location']) && $_SESSION['JSJOBS_SEARCH']['location'] != '') ? $_SESSION['JSJOBS_SEARCH']['location'] : null;
    ...
    function storeJob($data) {
        ...
        $data['qualifications'] = wpautop(wptexturize(stripslashes($_POST['qualifications'])));
        $data['prefferdskills'] = wpautop(wptexturize(stripslashes($_POST['prefferdskills'])));
        $data['agreement'] = wpautop(wptexturize(stripslashes($_POST['agreement'])));
        ...
        foreach ($ids as $id) {
            if ($this->jobCanDelete($id) == true) {
                ...
                if (!$row->delete($id)) {
                    ...
                    JSJOBSincluder::getJSModel('emailtemplate')->sendMail(2, 2, $id); // 2 for job,2 for DELETE job   
                }
            } else {
                $notdeleted += 1;
            }
...
//end
    ...
    function getMessagekey(){
        $key = 'job';if(is_admin()){$key = 'admin_'.$key;}return $key;
    }
