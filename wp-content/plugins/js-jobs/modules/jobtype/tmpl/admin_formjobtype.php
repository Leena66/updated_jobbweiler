...
<div id="jsjobsadmin-wrapper">
    ...
    <span class="js-admin-title">
        <a href="<?php echo admin_url('admin.php?page=jsjobs_jobtype'); ?>"><img src="<?php echo jsjobs::$_pluginpath; ?>includes/images/back-icon.png" /></a>
        <?php
        $heading = isset(jsjobs::$_data[0]) ? __('Edit', 'js-jobs') : __('Add New', 'js-jobs');
        echo $heading . '&nbsp' . __('Job Type', 'js-jobs');
        ?>
    </span>

    <form id="jsjobs-form" method="post" action="<?php echo admin_url("admin.php?page=jsjobs_jobtype&task=savejobtype"); ?>">
        ...
        <?php echo JSJOBSformfield::hidden('action', 'jobtype_savejobtype'); ?>
...
<?php echo JSJOBSformfield::hidden('form_request', 'jsjobs'); ?>
        <div class="js-submit-container js-col-lg-8 js-col-md-8 js-col-md-offset-2 js-col-md-offset-2">
            <a id="form-cancel-button" href="<?php echo admin_url('admin.php?page=jsjobs_jobtype'); ?>" ><?php echo __('Cancel', 'js-jobs'); ?></a>
<?php echo JSJOBSformfield::submitbutton('save', __('Save','js-jobs') .' '. __('Job Type', 'js-jobs'), array('class' => 'button')); ?>
        </div>
    </form>
</div>
</div>