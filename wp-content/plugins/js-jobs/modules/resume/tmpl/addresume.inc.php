...
//wp_enqueue_script('multi-files-selector', jsjobs::$_pluginpath . 'includes/js/multi-files-selector.js');
...
<script type="text/javascript">
    ...
    function loadMap( sectionid ) {
        ...
        var default_latitude = "<?php echo jsjobs::$_configuration['default_latitude']; ?>";
        var default_longitude = "<?php echo jsjobs::$_configuration['default_longitude']; ?>";
    ...
    function getVisible() {    
        ...
        }else if(jQuery('div#jsjb-jm-reume-form-wrap').length){
            var div = jQuery('div#jsjb-jm-reume-form-wrap');
        }
    ...
    function cancelJobApplyVisitor(){
        ...
        if(result == true){
            jQuery.post(ajaxurl,{action: 'jsjobs_ajax', jsjobsme: 'jobapply', task: 'canceljobapplyasvisitor'},function(data){
                if(data){
    ...
    function JobApplyVisitor(){
        ...
        if(resumeid == -1){
            ...
            jQuery.post(ajaxurl,{action: 'jsjobs_ajax', jsjobsme: 'jobapply', task: 'visitorapplyjob'},function(data){
               if(data){
                   window.location = data;
               } 
           });
    ...
    jQuery(document).ready(function(){
        ...
        if(jQuery('div#jsjobs-wrapper').length){
            ...
            maindivoffsettop = jQuery('div#jsjb-jm-reume-form-wrap').offset().top;
        }
    ...
    function showResumeFilesArrayPopup(){
        ...
        for (i = 0; i < resumefiles.length; i++){
            ...
            if (obj.canupload == 1){
                var mHTML = '<a href="javascript:void(0);" onclick="removeFileByIndex(' + i + ');" class="file"><span class="filename">' + obj.file.name + '</span><span class="fileext">&nbsp;</span><img class="filedownload" src="<?php echo jsjobs::$_pluginpath; ?>includes/images/resume/cancel.png" /></a>';
    ...
    function deleteResumeFile(id){
        ...
        jQuery.post(ajaxurl, {jsjobsme:'resume', action:'jsjobs_ajax', task:'removeResumeFileById', id:id}, function (data){
            if (data){
    ...
    jQuery("body").delegate('a.add', 'click', function(e){
        ...
        jQuery.post(ajaxurl, {action: 'jsjobs_ajax', jsjobsme: 'resume', task: 'getResumeSectionAjax', section: section, resumeid: resumeid}, function(data){
    ...
    jQuery(document).ready(function () {
        ...
        jQuery("body").delegate('span.resume-moreoptiontitle', 'click', function(e){
            ...
            if (jQuery('div.resume-moreoption').is(':hidden')) {
                var srcimg = '<?php echo jsjobs::$_pluginpath . 'includes/images/resume/up.png'; ?>';
            } else{
                var srcimg = '<?php echo jsjobs::$_pluginpath . 'includes/images/resume/down.png'; ?>';
            }
        ...
        jQuery("body").delegate('div.section_wrapper a.edit', 'click', function(e){
            ...
            jQuery.post(ajaxurl, {action: 'jsjobs_ajax', jsjobsme: 'resume', task: 'getResumeSectionAjax', section: section, sectionid:sectionid, resumeid: resumeid}, function(data){
        ...
        jQuery("body").delegate('div.section_wrapper a.delete', 'click', function(e){            
            ...
            var resumeid = jQuery('input#resume_temp').val();
            jQuery('a[data-section="' + section + '"]').remove();
            jQuery.post(ajaxurl, {action: 'jsjobs_ajax', jsjobsme: 'resume', task: 'deleteResumeSectionAjax', section: section, sectionid:sectionid, resumeid: resumeid}, function(data){
...
function submitresume(){
    ...
    function getTokenInputTags(multitags) {
        ...
        jQuery("#tags").tokenInput(tagArray, {
            ...
            <?php $newtyped_tags = JSJOBSincluder::getJSModel('configuration')->getConfigurationByConfigName('newtyped_tags');
            if ($newtyped_tags == 1) { ?>
                onResult: function(item) {
                    if (jQuery.isEmptyObject(item)){
        ...
        onAdd: function(item) {
            if (item.id != ''){return; }
            var ajaxurl = "<?php echo admin_url('admin-ajax.php') ?>";
            jQuery.post(ajaxurl, {action: 'jsjobs_ajax', jsjobsme: 'tag', task: 'saveTokenInputTag', tagdata: jQuery("tester").text()}, function(data){
                if (data){
                    try {
                        var value = jQuery.parseJSON(data);
    ...
    jQuery(document).ready(function(){
        ...
        if (print_link) {
            ...
            var href = '<?php echo jsjobs::makeUrl(array('jsjobsme'=>'resume', 'jsjobslt'=>'printresume', 'jsjobsid'=>$resumeid, 'jsjobspageid'=>jsjobs::getPageid())); ?>';
