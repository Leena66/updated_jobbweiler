...
class JSJOBSResumeModel {
    ...
    function storeResume($data){
        ...
        /*
        echo '<pre>';var_dump($data['sec_1']['ufield_408']);
        echo '<pre>';print_r($resumedata);echo '</pre>';
        exit;
        */
        ...
        if (isset($_SESSION['jsjobs_apply_visitor'])) {
            $url = JSJOBSincluder::getJSModel('jobapply')->visitorapplyjob();
            wp_redirect($url);
            exit;
        }        
        return SAVED;
    ...
    function storePersonalSection($data){
        ...
        if (JSJOBSincluder::getObjectClass('user')->isguest()) {
            ...
            if ($visitor_can_add_resume == 1) {
                $_SESSION['wp-jsjobs']['resumeid'] = $resumeid;
            }
        }
    ...
    function getAllEmpApps() {
        ...
        $searchjobsalaryrange = JSJOBSrequest::getVar('searchjobsalaryrange');
        ...
        if ($formsearch == 'JSJOBS_SEARCH') {
            $_SESSION['JSJOBS_SEARCH']['searchtitle'] = $searchtitle;
            $_SESSION['JSJOBS_SEARCH']['searchname'] = $searchname;
            $_SESSION['JSJOBS_SEARCH']['searchjobcategory'] = $searchjobcategory;
            $_SESSION['JSJOBS_SEARCH']['searchjobtype'] = $searchjobtype;
            $_SESSION['JSJOBS_SEARCH']['searchjobsalaryrange'] = $searchjobsalaryrange;
        ...
        if (JSJOBSrequest::getVar('pagenum', 'get', null) != null) {
            $searchtitle = (isset($_SESSION['JSJOBS_SEARCH']['searchtitle']) && $_SESSION['JSJOBS_SEARCH']['searchtitle'] != '') ? $_SESSION['JSJOBS_SEARCH']['searchtitle'] : null;
            ...
            $searchjobtype = (isset($_SESSION['JSJOBS_SEARCH']['searchjobtype']) && $_SESSION['JSJOBS_SEARCH']['searchjobtype'] != '') ? $_SESSION['JSJOBS_SEARCH']['searchjobtype'] : null;
            $searchjobsalaryrange = (isset($_SESSION['JSJOBS_SEARCH']['searchjobsalaryrange']) && $_SESSION['JSJOBS_SEARCH']['searchjobsalaryrange'] != '') ? $_SESSION['JSJOBS_SEARCH']['searchjobsalaryrange'] : null;
    ...
    function sorting() {
        ...
        jsjobs::$_data['sorton'] = JSJOBSrequest::getVar('sorton', 'post', 6);
        jsjobs::$_data['sortby'] = JSJOBSrequest::getVar('sortby', 'post', 2);
        if($pagenum > 1 && isset($_SESSION['resume'])){
            jsjobs::$_data['sorton'] = $_SESSION['resume']['sorton'];
            jsjobs::$_data['sortby'] = $_SESSION['resume']['sortby'];
        }else{
            $_SESSION['resume']['sorton'] = jsjobs::$_data['sorton'];
            $_SESSION['resume']['sortby'] = jsjobs::$_data['sortby'];
        }
        ...
        if (JSJOBSincluder::getObjectClass('user')->isguest()) {
            ...
            if ($conflag == 1) {
                if (isset($_SESSION['wp-jsjobs']) && isset($_SESSION['wp-jsjobs']['resumeid'])) {
                    if ($id == $_SESSION['wp-jsjobs']['resumeid']) {
                        return true;
                    } else {
                        return false;
                    }
    ...
    function cancelResumeSectionAjax() {
        ...
        switch ($section) {
            ...
            case 'personal':
                ...
                $result .= '<div class="resume-section-title personal"><img class="heading-img" src="' . jsjobs::$_pluginpath . 'includes/images/personal-info.png" />' . __('Personal information', 'js-jobs') . '</div>';
...
//END
    ...
    function getResumebyId($id) {
        ...
        if (($id != '') && ($id != 0)) {
            ...
            if($theme == 'Job Manager'){
                ...
                /*echo '<pre>';print_r($relatedresume);
                exit;*/
                ...
                /*echo "<pre>";var_dump(jsjobs::$_data['relatedresume']);        
                exit;*/
            }
        }
        jsjobs::$_data['resumecontactdetail'] = true;
    ...
    function getResumeFiles() {
        ...
        include_once(jsjobs::$_path . '/includes/resumeformlayout.php');
    ...
    function deleteResume($ids) {
        ...
        foreach ($ids as $id) {
            if ($this->resumeCanDelete($id) == true) {
                ...
                $_SESSION['resumetitle'] = $resumetitle;
                $_SESSION['jobseekername'] = $username;
                ...
                JSJOBSincluder::getJSModel('emailtemplate')->sendMail(3, 6, $id); // 3 for resume,6 for DELETE resume   
            }else{
                $notdeleted += 1;
            } 
        } 
    ...
    function resumeEnforceDelete($resumeid, $uid) {
        ...
        if (!jsjobsdb::query($query)) {
            return DELETE_ERROR; //error while delete resume
        }
        ...
        JSJOBSincluder::getJSModel('emailtemplate')->sendMail(3, 6, $resumeid); // 3 for resume,6 for DELETE resume
        return DELETED;
    }
    ...
    function getResumeInfoForEmail($resumeid) {
        ...
        $query = 'SELECT resume.application_title AS resumetitle, CONCAT(user.first_name," ",user.last_name) AS username
                        ...
                        FROM `' . jsjobs::$_db->prefix . 'js_job_resume` AS resume
                        JOIN `' . jsjobs::$_db->prefix . 'js_job_users` AS user ON user.id = resume.uid
                        WHERE resume.id = '.$resumeid;
    ...
    function getResumeByCategory() {
        ...
        foreach($categories AS $category){
            ...
            $query = "SELECT category.cat_title, CONCAT(category.alias,'-',category.id) AS aliasid,category.serverid
                ...
                where resume.job_category = category.id AND resume.status = 1 AND resume.searchable = 1 )  AS totaljobs
                FROM `" . jsjobs::$_db->prefix . "js_job_categories` AS category 
                WHERE category.isactive = 1 AND category.parentid = ".$category->categoryid." ORDER BY category.ordering ASC LIMIT ".$subcategory_limit;
    ...
    function getResumes($vars) {
        ...
        if (isset($jsformresumesearch) AND $jsformresumesearch == 1) {
            $_SESSION['jsjobs-searchresume-form'] = array();
        ...
        if (isset($vars['category']) AND $vars['category'] != '') {
            ...
            if (!is_numeric($categoryid))
                return false;
            $inquery = " AND resume.job_category = $categoryid";
        ...
        //variables form search form
            if(JSJOBSrequest::getVar('application_title')){
                $_SESSION['jsjobs-searchresume-form']['application_title'] = JSJOBSrequest::getVar('application_title');
            }
            $title = isset($_SESSION['jsjobs-searchresume-form']['application_title']) ? $_SESSION['jsjobs-searchresume-form']['application_title'] : '';
            if ($title != '') {
                $inquery .= ' AND resume.application_title LIKE "%' . $title . '%" ';
                jsjobs::$_data['filter']['application_title'] = $title;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('first_name')){
                $_SESSION['jsjobs-searchresume-form']['first_name'] = JSJOBSrequest::getVar('first_name');
            }
            $firstName = isset($_SESSION['jsjobs-searchresume-form']['first_name']) ? $_SESSION['jsjobs-searchresume-form']['first_name'] : '';
            if ($firstName != '') {
                $inquery .= ' AND resume.first_name LIKE "%' . $firstName . '%" ';
                jsjobs::$_data['filter']['first_name'] = $firstName;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('middle_name')){
                $_SESSION['jsjobs-searchresume-form']['middle_name'] = JSJOBSrequest::getVar('middle_name');
            }
            $middleName = isset($_SESSION['jsjobs-searchresume-form']['middle_name']) ? $_SESSION['jsjobs-searchresume-form']['middle_name'] : '';
            if ($middleName != '') {
                $inquery .= ' AND resume.middle_name LIKE "%' . $middleName . '%" ';
                jsjobs::$_data['filter']['middle_name'] = $middleName;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('last_name')){
                $_SESSION['jsjobs-searchresume-form']['last_name'] = JSJOBSrequest::getVar('last_name');
            }
            $lastName = isset($_SESSION['jsjobs-searchresume-form']['last_name']) ? $_SESSION['jsjobs-searchresume-form']['last_name'] : '';
            if ($lastName != '') {
                $inquery .= ' AND resume.last_name LIKE "%' . $lastName . '%" ';
                jsjobs::$_data['filter']['last_name'] = $lastName;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('nationality')){
                $_SESSION['jsjobs-searchresume-form']['nationality'] = JSJOBSrequest::getVar('nationality');
            }
            $nationality = isset($_SESSION['jsjobs-searchresume-form']['nationality']) ? $_SESSION['jsjobs-searchresume-form']['nationality'] : '';
            if ($nationality != '') {
                $inquery .= ' AND resume.nationality =' . $nationality . '';
                jsjobs::$_data['filter']['nationality'] = $nationality;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('gender')){
                $_SESSION['jsjobs-searchresume-form']['gender'] = JSJOBSrequest::getVar('gender');
            }
            $gender = isset($_SESSION['jsjobs-searchresume-form']['gender']) ? $_SESSION['jsjobs-searchresume-form']['gender'] : '';
            if ($gender != '') {
                $inquery .= ' AND resume.gender LIKE "%' . $gender . '%" ';
                jsjobs::$_data['filter']['gender'] = $gender;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('jobtype')){
                $_SESSION['jsjobs-searchresume-form']['jobtype'] = JSJOBSrequest::getVar('jobtype');
            }
            $jobType = isset($_SESSION['jsjobs-searchresume-form']['jobtype']) ? $_SESSION['jsjobs-searchresume-form']['jobtype'] : '';
            if ($jobType != '') {
                $inquery .= ' AND resume.jobtype = ' . $jobType . ' ';
                jsjobs::$_data['filter']['jobtype'] = $jobType;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('currencyid')){
                $_SESSION['jsjobs-searchresume-form']['currencyid'] = JSJOBSrequest::getVar('currencyid');
            }
            $currencyId = isset($_SESSION['jsjobs-searchresume-form']['currencyid']) ? $_SESSION['jsjobs-searchresume-form']['currencyid'] : '';
            if ($currencyId != '') {
                $inquery .= ' AND resume.currencyid =' . $currencyId . ' ';
                jsjobs::$_data['filter']['currencyid'] = $currencyId;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('salaryrangefrom')){
                $_SESSION['jsjobs-searchresume-form']['salaryrangefrom'] = JSJOBSrequest::getVar('salaryrangefrom');
            }
            $salaryRangeFrom = isset($_SESSION['jsjobs-searchresume-form']['salaryrangefrom']) ? $_SESSION['jsjobs-searchresume-form']['salaryrangefrom'] : '';
            if ($salaryRangeFrom != '') {
            ...
            if(JSJOBSrequest::getVar('salaryrangeend')){
                $_SESSION['jsjobs-searchresume-form']['salaryrangeend'] = JSJOBSrequest::getVar('salaryrangeend');
            }
            $salaryRangeend = isset($_SESSION['jsjobs-searchresume-form']['salaryrangeend']) ? $_SESSION['jsjobs-searchresume-form']['salaryrangeend'] : '';
            if ($salaryRangeend != '') {
                $inquery .= ' AND (SELECT rangestart  FROM  ' . jsjobs::$_db->prefix . 'js_job_salaryrange WHERE id = ' . $salaryRangeend . ' ) <= salaryrangeend.rangeend ';
                jsjobs::$_data['filter']['salaryrangeend'] = $salaryRangeend;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('salaryrangetype')){
                $_SESSION['jsjobs-searchresume-form']['salaryrangetype'] = JSJOBSrequest::getVar('salaryrangetype');
            }
            $salaryRangeType = isset($_SESSION['jsjobs-searchresume-form']['salaryrangetype']) ? $_SESSION['jsjobs-searchresume-form']['salaryrangetype'] : '';
            if ($salaryRangeType != '') {
                $inquery .= ' AND resume.jobsalaryrangetype = ' . $salaryRangeType . '  ';
                jsjobs::$_data['filter']['salaryrangetype'] = $salaryRangeType;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('highesteducation')){
                $_SESSION['jsjobs-searchresume-form']['highesteducation'] = JSJOBSrequest::getVar('highesteducation');
            }
            $highestEducation = isset($_SESSION['jsjobs-searchresume-form']['highesteducation']) ? $_SESSION['jsjobs-searchresume-form']['highesteducation'] : '';
            if ($highestEducation != '') {
                $inquery .= ' AND resume.heighestfinisheducation = ' . $highestEducation . ' ';
                jsjobs::$_data['filter']['highesteducation'] = $highestEducation;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('experience')){
                $_SESSION['jsjobs-searchresume-form']['experience'] = JSJOBSrequest::getVar('experience');
            }
            $totalExperience = isset($_SESSION['jsjobs-searchresume-form']['experience']) ? $_SESSION['jsjobs-searchresume-form']['experience'] : '';
            if ($totalExperience != '') {
                $inquery .= ' AND resume.experienceid = ' . $totalExperience . ' ';
                jsjobs::$_data['filter']['experience'] = $totalExperience;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('category')){
                $_SESSION['jsjobs-searchresume-form']['category'] = JSJOBSrequest::getVar('category');
            }
            $category = isset($_SESSION['jsjobs-searchresume-form']['category']) ? $_SESSION['jsjobs-searchresume-form']['category'] : '';
            if ($category != '' && is_numeric($category)) {
                $inquery .= ' AND resume.job_category = ' . $category . ' ';
                jsjobs::$_data['filter']['category'] = $category;
                jsjobs::$_data['issearchform'] = 1;
            }
            if(JSJOBSrequest::getVar('zipcode')){
                $_SESSION['jsjobs-searchresume-form']['zipcode'] = JSJOBSrequest::getVar('zipcode');
            }
            $zipCode = isset($_SESSION['jsjobs-searchresume-form']['zipcode']) ? $_SESSION['jsjobs-searchresume-form']['zipcode'] : '';
            if ($zipCode) {
                jsjobs::$_data['filter']['zipcode'] = $zipCode;
            }

            if(JSJOBSrequest::getVar('keywords')){
                $_SESSION['jsjobs-searchresume-form']['keywords'] = JSJOBSrequest::getVar('keywords');
            }
            $keywords = isset($_SESSION['jsjobs-searchresume-form']['keywords']) ? $_SESSION['jsjobs-searchresume-form']['keywords'] : '';
            if ($keywords) {
            ...
            if (!empty($data)) {
                foreach ($data as $uf) {
                    if(JSJOBSrequest::getVar($uf->field, 'post')){
                        $_SESSION['jsjobs-searchresume-form'][$uf->field] = JSJOBSrequest::getVar($uf->field, 'post');
                    }
                    $session_userfield = isset($_SESSION['jsjobs-searchresume-form'][$uf->field]) ? $_SESSION['jsjobs-searchresume-form'][$uf->field] : '';
        ...
        switch ($for) {
            case 'keywords':
                ...
                if ($total > 5)
                    $total = 5;
                for ($i = 0; $i < $total; $i++) {
                    $qa[] = "resume.keywords LIKE '" . $array[$i] . "'";
                }
                break;
            case 'tags':
                $array = explode(',', $array);
    ...
    function getAllResumeFiles() {
        ...
        $data_directory = JSJOBSincluder::getJSModel('configuration')->getConfigurationByConfigName('data_directory');
        $path = jsjobs::$_path . $data_directory;
        if (!file_exists($path)) {
...
//        ob_clean();
    ...
    function getMessagekey(){
        $key = 'resume';if(is_admin()){$key = 'admin_'.$key;}return $key;
    }
