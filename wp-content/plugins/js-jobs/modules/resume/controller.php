...
class JSJOBSResumeController {
            ...
            switch ($layout) {
                case 'resumes':
                    ...
                    if($resume_view_type==2){ // switch list to grid show save serch 
                        //unset($_SESSION['jsjobs-searchresume-form']);
                    }
                ...
                case 'admin_viewresume':                    
                    ...
                    if (JSJOBSincluder::getObjectClass('user')->isjobseeker() || current_user_can('manage_options') || (JSJOBSincluder::getObjectClass('user')->isemployer() && $empflag == 1) || JSJOBSincluder::getJSModel('configuration')->getConfigurationByConfigName('visitorview_emp_viewresume') == 1 ) {
                        ...
                        if ((!is_numeric($resumeid) && $resumeid{0} . $resumeid{1} . $resumeid{2} == 'sc-') || $socialid != null) { // social
                            ...
                            if(isset($resumeowner) && $resumeowner == true){
                        ...
                        if (is_admin() || $guestallowed == 1 || $check == true) {
                            if ($guestallowed == 1) {
                                if (isset($_SESSION['wp-jsjobs']) && isset($_SESSION['wp-jsjobs']['resumeid'])) {
                                    jsjobs::$_data['resumeid'] = $_SESSION['wp-jsjobs']['resumeid'];
                                }
                            }
                ...
                case 'admin_formresumeuserfield':
                    ...
                    if ($ff == "")
                        $ff = $_SESSION['formresumeuserfield_ff'];
                    else
                        $_SESSION['formresumeuserfield_ff'] = $ff;
    ...
    function approveQueueResume() {
        ...
        $url = admin_url("admin.php?page=jsjobs_resume&jsjobslt=resumequeue");
        wp_redirect($url);
        die();
    }
    ...
    function resumeenforcedelete() {
        ...
        if ($callfrom == 1) {
            $url = admin_url("admin.php?page=jsjobs_resume&jsjobslt=resumes");
        } else {
            $url = admin_url("admin.php?page=jsjobs_resume&jsjobslt=resumequeue");
        }
        wp_redirect($url);
    ...
    function empappreject() {
        ...
        $msg = JSJOBSMessages::getMessage($result, 'resume');
        JSJOBSMessages::setLayoutMessage($msg['message'], $msg['status'],$this->_msgkey);
        wp_redirect(admin_url("admin.php?page=jsjobs_resume&jsjobslt=resumequeue"));
        die();
    }
    ...
    function saveresume() {
        ...
        if ($result == SAVED) {
            if (is_admin()) {
                $url = admin_url("admin.php?page=jsjobs_resume");
            } else {
                $url = jsjobs::makeUrl(array('jsjobsme'=>'resume', 'jsjobslt'=>'myresumes'));
            }
        } else {
            if (is_admin()) {
                $url = admin_url("admin.php?page=jsjobs_resume");
    ...
    function removeresume() {
        ...
        if (is_admin()) {
            ...
            $url = jsjobs::makeUrl(array('jsjobsme'=>'resume', 'jsjobslt'=>'myresumes', 'jsjobspageid'=>jsjobs::getPageid()));
        }
        wp_redirect($url);
        die();
    }
