...
class JSJOBSJsjobsModel {
    ...
    function getAdminControlPanelData() {
        ...
        for ($i = 29; $i >= 0; $i--) {
            ...
            jsjobs::$_data['stack_chart_horizontal']['data'] .= "['" . date_i18n('Y-M-d', strtotime($checkdate)) . "',";
...
//         return $return;
    ...
    function widgetLastWeekData() {
        ...
        jsjobs::$_data['widget']['startdate'] = date('d M, Y', strtotime($lastdate));
        jsjobs::$_data['widget']['enddate'] = date('d M, Y', strtotime($curdate));
        return true;
    }
...
//function to create popup of newest applied jobs
    ...
    function getStepTwoValidate() {
        $basepath = ABSPATH;
        JSJOBSincluder::getJSModel('common')->makeDir($basepath . '/tmp');
        jsjobs::$_data['dir'] = substr(sprintf('%o', fileperms(jsjobs::$_path)), -3);

        if (!is_writable(jsjobs::$_path)) {
            jsjobs::$_data['dir'] = 0;
        }
        jsjobs::$_data['tmp_dir'] = substr(sprintf('%o', fileperms($basepath . '/tmp')), -3);
    ...
    function getMessagekey(){
        $key = 'jsjobs';if(is_admin()){$key = 'admin_'.$key;}return $key;
    }
