...
<div id="jsjobsadmin-wrapper">
    ...
    <span class="js-admin-title">
        <a href="<?php echo admin_url('admin.php?page=jsjobs&jsjobslt=steptwo'); ?>"><img src="<?php echo jsjobs::$_pluginpath; ?>includes/images/back-icon.png" /></a>
        <?php echo __('Update', 'js-jobs'); ?>
    </span>
    <div id="jsjobs-content">
        <form action="<?php echo admin_url("admin.php?page=jsjobs&task=getversionlist&action=task"); ?>" method="POST" name="adminForm" id="adminForm"  >
                    ...
                    <?php if (in_array('curl', get_loaded_extensions())) { ?>
                        <div class="js_installer_formwrapper">
                            <div id="jsjob_installer_forminput">
                                <input style="height:35px;" id="transactionkey" name="transactionkey" class="inputbox required" value="<?php echo jsjobs::$_data['transactionkey'];?>" placeholder="<?php echo __('Authentication key','js-jobs'); ?>..." />
            ...
            <input type="hidden" name="producttype" id="producttype" value="<?php echo JSJOBSincluder::getJSModel('configuration')->getConfigurationByConfigName('producttype'); ?>" />
            <input type="hidden" name="productcode" id="productcode" value="jsjobs" />
            <input type="hidden" name="productversion" id="productversion" value="<?php echo JSJOBSincluder::getJSModel('configuration')->getConfigurationByConfigName('versioncode'); ?>" />
            <input type="hidden" name="count_config" id="count_config" value="<?php echo JSJOBSincluder::getJSModel('configuration')->getCountConfig();  ?>" />
        ...
        <?php 
            if(jsjobs::$_data['response'] != ''){
                $response = base64_decode(jsjobs::$_data['response']);
                $response = json_decode($response);
                if($response[0] !== true){ ?>
                    <div id="jsst_error_message" style="display: inline-block;"><?php echo $response[1]?></div><?php  
                }else{ ?>
