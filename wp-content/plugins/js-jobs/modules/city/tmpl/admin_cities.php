...
<div id="jsjobsadmin-wrapper">
    ...
    <span class="js-admin-title">
        ...
        <a class="js-button-link button" href="<?php echo admin_url('admin.php?page=jsjobs_city&jsjobslt=formcity'); ?>"><img src="<?php echo jsjobs::$_pluginpath; ?>includes/images/add_icon.png" /><?php echo __('Add New','js-jobs') .'&nbsp;'. __('City', 'js-jobs') ?></a>
    </span>
    <div class="page-actions js-row no-margin">
        ...
        <a class="js-bulk-link button multioperation" message="<?php echo JSJOBSMessages::getMSelectionEMessage(); ?>" confirmmessage="<?php echo __('Are you sure to delete','js-jobs').' ?'; ?>" data-for="removecity" href="#"><img src="<?php echo jsjobs::$_pluginpath; ?>includes/images/delete-icon.png" /><?php echo __('Delete', 'js-jobs') ?></a>

    </div>
    <form class="js-filter-form" name="jsjobsform" id="jsjobsform" method="post" action="<?php echo admin_url("admin.php?page=jsjobs_city&jsjobslt=cities&countryid=$countryid&stateid=$stateid"); ?>">
    ...
    if (!empty(jsjobs::$_data[0])) {
        ?>
        <form id="jsjobs-list-form" method="post" action="<?php echo admin_url("admin.php?page=jsjobs_city"); ?>">
            <table id="js-table">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    for ($i = 0, $n = count(jsjobs::$_data[0]); $i < $n; $i++) {
                        ...
                        $link = admin_url('admin.php?page=jsjobs_city&jsjobslt=formcity&jsjobsid=' . $row->id);
                        ?>			
                        <tr>
                            <td class="grid-rows">
                            ...
                            <td width="20%">
                                <?php if ($row->enabled == '1') { ?>
                                    <a href="<?php echo admin_url('admin.php?page=jsjobs_city&task=unpublish&action=jsjobtask&jsjobs-cb[]='.$row->id.$pageid); ?>">
                                        <img src="<?php echo jsjobs::$_pluginpath; ?>includes/images/yes.png" alt="Default" border="0" />
                                    </a>
                                <?php } else { ?>
                                    <a href="<?php echo admin_url('admin.php?page=jsjobs_city&task=publish&action=jsjobtask&jsjobs-cb[]='.$row->id.$pageid); ?>">
                            ...
                            <td class="action" width="20%">
                                ...
                                <a href="<?php echo admin_url('admin.php?page=jsjobs_city&task=removecity&action=jsjobtask&jsjobs-cb[]='.$row->id); ?>" onclick="return confirmdelete('<?php echo __('Are you sure to delete', 'js-jobs').' ?'; ?>');"><img src="<?php echo jsjobs::$_pluginpath; ?>includes/images/remove.png" title="<?php echo __('Delete', 'js-jobs'); ?>"></a>
                            </td>
                        </tr>
                        <?php
                    }
            ...
            <?php echo JSJOBSformfield::hidden('action', 'city_remove'); ?>
        ...
        $link[] = array(
                    'link' => 'admin.php?page=jsjobs_city&jsjobslt=formcity',
